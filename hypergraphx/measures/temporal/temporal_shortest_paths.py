...
def supra_adj(temporal_network, subtimes, unique_individuals, dataset):
    ...
    G = nx.DiGraph()
    for t in subtimes:
        nodes = [(t,n) for n in (temporal_network[t].get_nodes() if isinstance(temporal_network[t], hgx.Hypergraph) else temporal_network[t].nodes())]
    ...
    for i, t in enumerate(subtimes) :
        ...
        # for each node
        NODES = subnet.get_nodes() if isinstance(subnet, hgx.Hypergraph) else subnet.nodes()
        for n in NODES:
            ...
            while j<len(subtimes) and not found:
                t_j = subtimes[j]
                # print(i, j, n)
                active_in = (n in set(temporal_network[subtimes[j]].get_nodes())) if isinstance(temporal_network[subtimes[j]], hgx.Hypergraph) else (n in set(temporal_network[subtimes[j]].nodes())) 
                if active_in:
                    found = True
                    if 'FnF' in dataset:
    ...
    for i, t in enumerate(subtimes) :
        ...
        if isinstance(subnet, hgx.core.hypergraph.Hypergraph):   
            ...
            edges = subnet.edges()
            edges_to_add = []      
            for e in edges: # for each edge in the original snapshot, see where these nodes are active for the first time again
                n1 = (t, e[0])
                n2 = (t, e[1])
            ...
            G.add_edges_from(edges_to_add)  #don't add too soon otherwise double-count during second adding-in
        elif isinstance(subnet, hgx.Hypergraph):
            edges = list(tuple(el) for el in subnet.get_edges())
            raise NotImplementedError("Currently not yet extended to hypergraphs")
        else:
            raise TypeError(f"A graph of type {type(subnet)} is not supported for SA-embeddings")
    ...
    for t in subtimes:
        ...
        not_present_nodes_t = set.difference(*[unique_individuals, not_present_nodes_t]) # which persons exists, but didn't interact here?
...
def calc_size_of_single_path(hyperpath, temporal_network, dataset, option):
    ...
    for (t1, u), (t2, v) in zip(hyperpath[:-1], hyperpath[1:]):
        ...
        if (u==v) and (u not in temporal_network[t1].get_nodes()) and (v not in temporal_network[t1].get_nodes()):
...
def P4_calc_shortest_fastest_paths(start_node_labels, static_graph_embedded, verbose=False):
    """
...
def P5_calc_best_paths(start_node_labels, temp, sdict, fdict,
    ...
    for u, subdict in tqdm(sdict.items()):
        ...
        assert u in start_node_labels, "for the single_source_dijkstra algorithm, the nodes in the resulting dict MUST only be start_nodes"

        person1 = int(temp[temp['new-node']==u]['OG-node'].values[0])               # assert len(temp[temp['new-node']==u]
        for v, lenhoS in (zip(lengths.keys(), lengths.values())):
            pathhoS = paths[v]  
            person2 = int(temp[temp['new-node']==v]['OG-node'].values[0])           # assert len(temp[temp['new-node']==v]['OG-node'].values.tolist())==1
            ...
            assert len(pathhoF) >= len(pathhoS), "the shortest path must be topologically shorter than the fastest path"
            ...
            data = (integer_lbl_to_static_node_map[u], integer_lbl_to_static_node_map[v], 
                    lenhoS, lenhoF, 
                    pathhoS, pathhoF)
...
def P6_calc_avg_orders(SHORTEST_PATH_DATA, integer_lbl_to_static_node_map, TS, dataset, option):
    """
    SQ_ARR is of the form:
    {person1: {person2: [(n1, n2, lenhoS, lenhoF, pathhoS, pathhoF), ...]}}
    where n1, n2 are (t, node) tuples
    ...
    for person1 in tqdm(SHORTEST_PATH_DATA.keys()):
        ...
        for person2 in SHORTEST_PATH_DATA[person1].keys():
            ...
            fastest_time_path = [el for el in spotlight if el[1][0]==min(endpoints)][0]
            n1, n2, _, lhF, _, phF = fastest_time_path

            true_sp = [integer_lbl_to_static_node_map[v] for v in phF]
            sizesf, deltatf, redundanciesF = calc_size_of_single_path(hyperpath=true_sp, temporal_network=TS, dataset=dataset, option=option)
            ...
            sizess, deltats, redundanciesS = calc_size_of_single_path(hyperpath=true_sp, temporal_network=TS, dataset=dataset, option=option)
...
def embed_time_series_for_dataset(temporal_network, dataset_name, root, verbose=False):
    ...
    dyad_subnetwork = {t: temporal_network[t].subhypergraph_by_orders(sizes=[2], keep_nodes=True) for t in subtimes}

    # create subTS of true only-ho-interactions networks
    
    higher_orders = {t: list(set(H.get_orders()) - {1}) for t, H in temporal_network.items()}
    ho_only_subnetwork = {t: temporal_network[t].subhypergraph_by_orders(orders=higher_orders[t], keep_nodes=True) for t in subtimes}
    ...
    # Embed DY
    G_DY_static_OG = supra_adj(temporal_network=dyad_subnetwork, subtimes=subtimes, unique_individuals=unique_individuals, dataset=dataset_name)
    # Embed HO only
    G_only_HO_static_OG = supra_adj(temporal_network=proj_ho_only_subnetwork, subtimes=subtimes, unique_individuals=unique_individuals, dataset=dataset_name)
    

    # Map (t,n)-nodes to new integer node label
    assert (set(G_DY_static_OG.nodes)).intersection(set(G_HO_static_OG.nodes)) == set(G_DY_static_OG.nodes), "The dyad nodeset should be contained IN the HO nodeset (in the case where nx.empty_graph was used in supra-adj embedding)"

    assert (set(G_only_HO_static_OG.nodes)).intersection(set(G_HO_static_OG.nodes)) == set(G_only_HO_static_OG.nodes), "The ho-only nodeset should be contained IN the HO nodeset (in the case where nx.empty_graph was used in supra-adj embedding)"

    static_node_to_integer_lbl_map = {k:i for i, k in enumerate(set(G_HO_static_OG.nodes).union(set(G_DY_static_OG.nodes)))}
    # dict of {new_lbl: (t,n)}
    integer_lbl_to_static_node_map = {v:k for k, v in static_node_to_integer_lbl_map.items()}

    fname_map = f'{root}/paths_temporal/node_mappings.pck'
    pk.dump([static_node_to_integer_lbl_map, integer_lbl_to_static_node_map], open(fname_map, 'wb'))
    ...
    G_HO_static = nx.relabel_nodes(G_HO_static_OG, mapping=static_node_to_integer_lbl_map)
...
def calc_redundancy_info(SQ_ARR):
    ...
    redundancy_df = pd.DataFrame(columns=['perc_of_path_is_redund_S', 'perc_of_path_is_redund_F', 
                                          'avg_num_redund_S', 'avg_num_redund_F', 
                                          'lS', 'lF'])
    for n1 in SQ_ARR.keys():
        for n2 in SQ_ARR[n1].keys():
            ...
            redundancy_df.loc[f"{n1}_{n2}"] = [perc_redund_for_single_path(redS), perc_redund_for_single_path(redF), 
                                            np.array(redS).mean(), np.array(redF).mean(),
    ...
    for opt in ['S', 'F']:
        df = redundancy_df[[f'perc_of_path_is_redund_{opt}', f'avg_num_redund_{opt}',f'l{opt}']].groupby(f'l{opt}').agg({f'perc_of_path_is_redund_{opt}':['mean', 'std'], 
                                                                                            f'avg_num_redund_{opt}':['mean', 'std']})
        df.columns = [f'perc_of_path_is_redund_{opt}_mean', f'perc_of_path_is_redund_{opt}_std',
                                f'avg_num_redund_{opt}_mean', f'avg_num_redund_{opt}_std']
        df = df.reset_index().join(redundancy_df.groupby(f'l{opt}').agg({f'l{opt}':'count'}), on=f'l{opt}', rsuffix='_sum').set_index(f'l{opt}').sort_index()
        dataframes[opt] = df
...
def P7_construct_square_arrays(SHORTEST_PATH_DATA, V, Vt0):
    ...
    for person1 in tqdm(SHORTEST_PATH_DATA.keys()):
        for person2 in SHORTEST_PATH_DATA[person1].keys():
            ...
            dyad_prop = [sum(el==2 for el in sizess) / len(sizess), 
    ...
    outputs = PATH_LEN_ARRAY[Vt0], AVG_ORD_ARRAY[Vt0], PATH_TIME_ARRAY[Vt0], DYADIC_PROP_ARRAY[Vt0], BEST_TIME_ARRAY[Vt0]
...
def P8_save_square_arrays(outputs, option, regime, verbose, dsdir):
    PATH_LEN_ARRAY_HO, AVG_ORD_ARRAY_HO, PATH_TIME_ARRAY_HO, DYADIC_PROP_ARRAY_HO, BEST_TIME_ARRAY_HO = outputs

    # if verbose: print(f"len(Vt0) = {len(Vt0)} for {regime}")

    # SAVING ALL FILES
    arrnames = ['SPL', 'FPL',   #Shortest-Path-Length, Fastest-Path-Length
                'STL', 'FTL',   #Shortest-Time-Length, Fastest-Time-Length
                'SAO', 'FAO',   #Shortest-Avg-Order, Fastest-Avg-Order
                'SPropDy', 'FPropDy', #Proportion-of-Dyadic-Steps-in-Shortest/Fastest-Path
                'BestTime']     #Whether-Shortest-and-Fastest-Path-are-the-Same-Timewise
    

    arrnames = [f"{el}_{regime}" for el in arrnames]

    arrays = [PATH_LEN_ARRAY_HO[:,:,0], PATH_LEN_ARRAY_HO[:,:,1], 
            PATH_TIME_ARRAY_HO[:,:,0], PATH_TIME_ARRAY_HO[:,:,1],
            AVG_ORD_ARRAY_HO[:,:,0], AVG_ORD_ARRAY_HO[:,:,1], 
            DYADIC_PROP_ARRAY_HO[:,:,0], DYADIC_PROP_ARRAY_HO[:,:,1], 
            BEST_TIME_ARRAY_HO]
...
def calc_shortest_fastest_paths_temporal_hypergraphs(root=None,
                                                    dataset_name=None,
                                                    verbose=False, 
                                                    option='min', 
                                                    regime='HO'):
    ...
    assert option in ['min', 'max', 'mean'], "option must be one of ['min', 'max', 'mean']"
    assert regime in ['HO', 'DY', 'HOonly'], "regime must be one of ['HO', 'DY', 'HOonly']"
    ...
    fname_map = f'{root}/paths_temporal/node_mappings.pck'   
    static_node_to_integer_lbl_map, integer_lbl_to_static_node_map = pk.load(open(fname_map, 'rb'))   
