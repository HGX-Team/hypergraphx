...
def compute_all_edges_shortest_path(H, graph_distance=None, verbose=False, aggregate=False):
    ...
    if graph_distance is None:
        if verbose:
            print('Computing node shortest path distance of the aggregated projected network...')
        # Compute node shortest path distances for the aggregated projected network
        graph_distance = compute_all_nodes_shortest_path(projections.clique_projection(H_agg))
        if verbose:
            print('Complete!')

    if verbose:
        print('Computing topological distance of hyperlinks path distance of the higher order aggregated network...')
        ...
        for edge2 in H_agg.get_edges():
            if (edge1, edge2) not in edge_dic_distance:
                if edge1 == edge2:
                    ...
                    edge_dic_distance[(edge1, edge2)] = min(
                        [graph_distance[x][y] for x, y in itertools.product(edge1, edge2)]) + 1
                    edge_dic_distance[(edge2, edge1)] = edge_dic_distance[(edge1, edge2)]
...
def get_mean_distance_events(H, order, edge_distance=None, cross_order=False):
    ...
    if cross_order:
        ...
        #events of order d'
        size_other_order = {edge: size[edge] for edge in size.keys() if len(edge) != order}
        for x, y in itertools.product(list(size_order.keys()), list(size_other_order.keys())):
        ...
        assert sum(list(cnt_ev1.values())) == sum(list(size_order.values())) * sum(list(size_other_order.values()))
...
def topological_temporal_distance_diff_order(H, df, order, distance_dict, dt_list, skip_inf = True, verbose=False):
    """
    ...
    tot_dic_dst_counter_gen['overall_avg_dist'] = np.average(list(cnt_ev1.keys()), weights=list(cnt_ev1.values()))
    n5 = df.shape[0]
    ...
    for p, delta_t in enumerate(dt_list):
        ...
        for ref_time in ref_list:
            ...
            df_rest = df[(df.timestamp == ref_time) | (
                        (df.timestamp >= ref_time + delta_t_low) & (df.timestamp < ref_time + delta_t))]
            ...
            ref_nodes_other_order = df_rest_other_order[df_rest_other_order.timestamp == ref_time].nodes
            ...
            df_rest_other_order = df_rest_other_order[df_rest_other_order.timestamp > ref_time]
            ...
            size_n_other_order = df_rest_other_order.groupby(df_rest_other_order.nodes).size()
            ...
            for x, y in itertools.product(ref_nodes_order, df_rest_other_order.nodes.unique()):
                cnt_dst.update({distance_dict[(x, y)]: size[y]})
            for x, y in itertools.product(ref_nodes_other_order, df_rest_order.nodes.unique()):
                cnt_dst.update({distance_dict[(x, y)]: size[y]})
        ...
        if delta_t == t_inf:
            assert np.average(list(cnt_dst.keys()), weights=list(cnt_dst.values())) == tot_dic_dst_counter_gen[
                'overall_avg_dist']
...
def topological_temporal_distance_same_order(H, df, order, distance_dict, dt_list, skip_inf = True, verbose=False):
    """
    ...
    for p, delta_t in enumerate(dt_list):
        ...
        for ref_time in ref_list:
            ...
            if (ref_time <= max_time):
                ...
                # The original set of events includes events at time t and those with delay delta_t',
                # such that delta_t_low <= delta_t' < delta_t.
                df_rest = df[(df.timestamp == ref_time) | ((df.timestamp >= ref_time + delta_t_low) & (df.timestamp < ref_time + delta_t))]
        ...
        # Check convergence: lim delta_t -> inf E[eta|delta_t] = E[eta]
        if delta_t == t_inf:
            assert np.average(list(cnt_dst.keys()), weights=list(cnt_dst.values())) == tot_dic_dst_counter_gen['overall_avg_dist']
...
def topological_temporal_cond_distance(H, order, distance_dict=None, same_order=True, fit_correlation = True, drop_duplicates=True, dt_list=None, skip_inf = True, verbose=False):
    """
    ...
    df = _to_df(H).sort_values('timestamp', ascending=True).reset_index().drop('index', axis=1)
    ...
    if same_order:
        cond_distr =  topological_temporal_distance_same_order(H, df, order, distance_dict, dt_list, skip_inf,verbose=verbose)
    else:
        cond_distr = topological_temporal_distance_diff_order(H, df, order, distance_dict, dt_list, skip_inf, verbose=verbose)
    gen_dict['avg_top_dist'] = cond_distr['overall_avg_dist']
    gen_dict['cond_top_dist_distribution'] = cond_distr['cond_dist']
    gen_dict['avg_cond_top_dist'] =  {k: np.average(list(v.keys()),weights = list(v.values()))/gen_dict['avg_top_dist'] for k,v in cond_distr['cond_dist'].items()}
    ...
    if fit_correlation:
        ...
        coef = np.polyfit(np.log(avg_cond_top_dist_sr.index), avg_cond_top_dist_sr.values, 1)
