...
def calc_HO_shortest_paths(Hbase:hgx.Hypergraph, 
                           option='min', 
                           root=None, 
                           verbose=False):
    '''
    ...
    assert(min(list(Hbase.get_nodes())) == 0), f"the network should have nodes starting from 0!, but instead they start from {min(list(H__base.get_nodes()))} \nHas the stattic graph for {ds} been correctly calculated from the TN?"
    ...
    H_onlyHO = Hbase.subhypergraph_by_orders(sizes=range(3, Hbase.max_size()+1), keep_nodes=True)
...
def calc_HO_shortest_paths_from_file(fnameho, option='min', root=None, verbose=False):
    ...
    SPL_ho, SPL_dy, SPL_onlyho, avg_ord, shortest_paths_ho_dict_enriched = calc_HO_shortest_paths(Hbase=Hbase,
                                                                        option=option,
                                                                        root=root,
                                                                        verbose=verbose)
...
def calc_ho_shortest_paths(H:hgx.Hypergraph):
    ...
    SPL_ho = dict_to_df(dict_sps=shortest_paths_ho, 
                        nodes=H.get_nodes())
    
    return shortest_paths_ho, SPL_ho


def calc_sizes_redundancies_of_shortest_paths(shortest_paths_ho, Hbase, option, root=None):
    """
    ...
    for ctr, (node_i, dicto) in enumerate(shortest_paths_ho.items()):
        ...
        for node_j, shortest_path in dicto.items():
            ...
            if node_i==node_j:
                ...
                for ctr, (k,l) in enumerate(zip(shortest_path[:-1], shortest_path[1:])):
                    # find all edges containing both nodes i and j
                    incident_edges = set(Hbase.get_incident_edges(node=k)).intersection(set(Hbase.get_incident_edges(node=l)))
                    ...
                    redunancies[ctr] = len(incident_edges)-1  # no.incident edges = 1 + no.redundant edges

            # expand the shortest_path_ho dict to include path orders and redundancies
            shortest_paths_ho[node_i][node_j] = {'sp':shortest_paths_ho[node_i][node_j], 
                                                 'sizes':path_sizes, 
...
def calc_prop_true_dyad_paths_per_spl(SPL_ho, avg_ord):
    ...
    temp = pd.DataFrame(data=[SPL_ho.ravel(), avg_ord.ravel()], index=['spl', 'avg_ord']).T
...
def calc_prop_of_each_path_is_dyad(shortest_paths_ho, SPL_ho):
    ...
    temp = pd.DataFrame(data=[SPL_ho.to_numpy().ravel(), prop_dyad_path.ravel()], index=['spl', 'pd']).T
    temp = temp[temp['spl']!=0]
