...
from hypergraphx.generation._rng import py_rng


def directed_configuration_model(
    hypergraph: DirectedHypergraph, *, seed: int | None = None
) -> DirectedHypergraph:
    """
    ...
    --------
    >>> from hypergraphx import DirectedHypergraph
    >>> from hypergraphx.generation import directed_configuration_model
    >>> H = DirectedHypergraph(edge_list=[((0,), (1, 2)), ((2,), (0,))], weighted=False)
    >>> H2 = directed_configuration_model(H, seed=0)
    ...
    for _ in range(num_steps_sources):
        id1 = rng.randint(0, len(new_hyperedges) - 1)
        id2 = rng.randint(0, len(new_hyperedges) - 1)
        ...
        node1 = rng.choice(source1)
        # select random node from source2
        node2 = rng.choice(source2)
    ...
    for _ in range(num_steps_targets):
        ...
        node1 = rng.choice(target1)
        # select random node from target2
        node2 = rng.choice(target2)
