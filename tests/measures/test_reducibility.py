...
def test_reducibility_basic():
    ...
    eta, reps = reducibility(hg, optimization="exact", ent_method="count")
    assert 0.0 <= eta <= 1.0
...
def test_reducibility_partition_dict():
    ...
    eta, reps = reducibility(hg, partition=partition, ent_method="count")
    assert 0.0 <= eta <= 1.0
    assert max(reps) == max(hg.get_sizes())
...
def test_reducibility_greedy():
    hg = _small_hypergraph()
    eta, reps = reducibility(hg, optimization="greedy", ent_method="count")
    assert 0.0 <= eta <= 1.0
    assert max(reps) == max(hg.get_sizes())
...
def test_reducibility_invalid_ent_method():
    ...
    with pytest.raises(InvalidParameterError):
        reducibility(hg, ent_method="invalid")
...
def test_partition_list_wrong_length():
    hg = _small_hypergraph()
    with pytest.raises(InvalidParameterError):
        reducibility(hg, partition=[0, 1], ent_method="count")


def test_partition_dict_missing_node():
    hg = _small_hypergraph()
    with pytest.raises(InvalidParameterError):
        reducibility(hg, partition={0: 0, 1: 0, 2: 1}, ent_method="count")
