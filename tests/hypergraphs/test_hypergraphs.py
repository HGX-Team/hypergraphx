...
def test_hypergraph_initialization_with_metadata():
    ...
    hg = Hypergraph(node_metadata=node_metadata, hypergraph_metadata=hypergraph_metadata)
...
def test_add_edge_unweighted():
    ...
    assert hg._weights[hg._edge_list[(1, 2, 3)]] is None
...
def test_add_edge_duplicate_edge_weighted():
    ...
    hg.add_edge((1, 2, 3), weight=2.5)
    assert hg._weights[hg._edge_list[(1, 2, 3)]] == 2.5  # Weight is updated to the latest value
...
def test_add_edge_with_metadata():
    ...
    assert hg.edge_metadata[(1, 2, 3)] == metadata
...
def test_add_edges_unweighted():
    ...
    hg.add_edges(edges)
    for edge in edges:
        assert edge in hg._edge_list
    assert all(hg._weights[hg._edge_list[edge]] is None for edge in edges)
...
def test_add_edges_weighted():
    ...
    for edge, weight in zip(edges, weights):
        assert edge in hg._edge_list
        assert hg._weights[hg._edge_list[edge]] == weight
...
def test_add_edges_metadata():
    ...
    for edge, meta in zip(edges, metadata):
        assert hg.edge_metadata[edge] == meta
...
def test_add_edges_repeated_edges_with_weights():
    ...
    edges = [(1, 2), (1, 2)]
    weights = [1.0, 2.0]
    with pytest.raises(ValueError, match="If weights are provided, the edge list must not contain repeated edges."):
        hg.add_edges(edges, weights=weights)
...
def test_add_edges_mismatch_weights_and_edges():
    ...
    edges = [(1, 2), (3, 4)]
    weights = [1.0]
    with pytest.raises(ValueError, match="The number of edges and weights must be the same."):
        hg.add_edges(edges, weights=weights)
...
def test_remove_node_keep_edges():
    ...
    assert (4,) in hg._edge_list  # Incident edge reduced to (4,)
...
def test_remove_node_with_self_loop():
    """Test removing a node that is part of a self-loop."""
    hg = Hypergraph()
    hg.add_edge((1, 1))
    assert (1, 1) in hg._edge_list
    assert 1 in hg._adj
    assert hg._adj[1] == {hg._edge_list[(1, 1)]}
    hg.remove_node(1)
    assert (1, 1) not in hg._edge_list
    assert 1 not in hg._adj
...
def test_get_edges_keep_nodes_without_subhypergraph_error():
    ...
    hg = Hypergraph()
    hg.add_edge((1, 2, 3))
    with pytest.raises(ValueError, match="Cannot keep nodes if not returning subhypergraphs."):
